import { Image, Notes, Appear } from 'mdx-deck'
import { dark } from 'mdx-deck/themes'
import Footer from 'mdx-deck-footer'
import { Split as S } from 'mdx-deck/layouts'
import codeSurfer from 'prism-react-renderer/themes/nightOwl'
export { components } from 'mdx-deck-code-surfer'
const Layout = ({ children }) => (
  <Footer conf="Alicante Frontend 🌊" speaker="@FlavioCorpa">
    {children}
  </Footer>
)
const Split = ({ children, ...props }) => (
  <Footer>
    <S {...props}>{children}</S>
  </Footer>
)
export const theme = {
  ...dark,
  codeSurfer,
  monospace: '"Operator Mono Lig", "Dank Mono", "Fira Code", Consolas, "Roboto Mono", monospace',
  colors: {
    text: 'rgb(214, 222, 235)',
    code: 'rgb(130, 170, 255)',
    link: 'rgb(128, 203, 196)',
    background: 'rgb(1, 22, 39)',
  },
}

<Image src={require('file-loader!./img/cover.jpeg')} />

---

export default Layout

`$ whoami`

---
export default Layout

<Image src={require('file-loader!./img/me.png')} />

---

export default Layout

### FP in JavaScript? Sure!

<img src={require('file-loader!./img/fox.png')} />

---

export default Layout

## What is Elm? 🌳

<img width="300px" src={require('file-loader!./img/elm-logo.png')} />

<Notes>
  <li>Friendly error messages</li>
  <li>Small subset of Haskell, simpler than JavaScript</li>
  <li>Static Types, pure Functional programming</li>
  <li>NO RUNTIME ERRORS!</li>
  <li>Interop via ports/flags and web components</li>
  <li>Great tooling and community</li>
</Notes>


---
export default Layout

#  Elm is a pure functional programming language

<Appear>
  <li><i>This means no uncontrolled side-effects</i></li>
  <li>Elm has eager evaluation (unlike Haskell which is <i>lazy</i>)</li>
  <li><strong>NO mutability 💃🏻</strong></li>
  <li>Algebraic Data Types and pattern matching! 👑</li>
  <li>Static Types with AMAZING type inference (ML family), sorry TypeScript...</li>
  <li>Resilient, maintainable, scalable, testable code</li>
</Appear>

---
export default Layout

## JavaScript vs. Elm Cheatsheet

---
export default Layout

<img width="1500px" src={require('file-loader!./img/syntax1.png')} />

---
export default Layout

<img width="1500px" src={require('file-loader!./img/syntax2.png')} />

---
export default Layout

<img width="1500px" src={require('file-loader!./img/syntax3.png')} />

---

export default Layout

## Elm is small! 🤏🏼

<Image src={require('file-loader!./img/elm-cheat.png')} />

---

export default Layout

## ADTs! (_variants_ in Reason)

---

export default Layout

```reason Variants! 🙌🏼
type state =
  | Error
  | Loading
  | Loaded(Route.t, list(Post.t));

type action =
  | FetchPosts
  | FetchComments(string)
  | FailedToFetch
  | FetchedPosts(list(Post.t))
  | FetchedComments(string, list(Comment.t))
  | AddComment(string, string, string)
  | DeleteComment(string, string)
  | Like(Post.t, bool)
  | ChangeRoute(Route.t);
----
* > ❤️
2:3 > Sum Types!
4,8,10:15 > Product Types!!
```

---

export default Layout

```reason Records! 😲
type post = {
  id: string,
  caption: Caption.t,
  images: Images.t,
  likes: Num.t,
  num_comments: Num.t,
  user_has_liked: bool,
  comments: list(Comment.t),
};
----
* > Syntactic sugar for Product Types! 💄
8 > types look a bit like functions `list()`
8[6] > Module.t (type) is a convention 😉
```

---

export default Layout

## Things have not always been perfect from the beginning...

---

export default Layout

```reason Url Parsing...
import Url.Parser exposing (Parser, (</>), int, map, oneOf, s, string)

type Route
  = Topic String
  | Blog Int
  | User String
  | Comment String Int

routeParser : Parser (Route -> a) a
routeParser =
  oneOf
    [ map Topic   (s "topic" </> string)
    , map Blog    (s "blog" </> int)
    , map User    (s "user" </> string)
    , map Comment (s "user" </> string </> s "comment" </> int)
    ]
----
* > Recommended way of dealing with routes requires parser combinators...
9 > Look at this weird type signature! 🤯
```

---

export default Layout

```reason Enter elm-app-url!
import AppUrl exposing (AppUrl)

type Route
  = Topic String
  | Blog Int
  | User String
  | Comment String Int

parse : AppUrl -> Maybe Route
parse url =
    case url.path of
        [ "topic", topic ] ->
            Just <| Topic topic

        [ "blog", blogId ] ->
            Maybe.map Blog <| String.toInt blogId

        [ "user", user ] ->
            Just <| User user

        [ "blog", user, "comment", commentId ] ->
            Maybe.map (Comment user) <| String.toInt commentId

        _ ->
            Nothing
----
* > Much better API (IMHO, looks like ReasonML)
9:25 > Parsing routes became just a simple plattern match! 🪄
```

---

export default Layout

```reason Pattern matching! 🙌🏼
let { path } = ReasonReactRouter.useUrl();

switch (path) {
| ["book", id, "edit"] => handleBookEdit(id)
| ["book", id] => getBook(id)
| ["book", id, _] => noSuchBookOperation()
| [] => showMainPage()
| ["shop"]
| ["shop", "index"] => showShoppingPage()
| ["shop", ...rest] => nestedMatch(rest)
| _ => showNotFoundPage()
};
----
* > The Bread & Butter of ReasonML! 🥪
1 > JS "destructuring" is just a pattern match
3, 12 > `switch` on steroids! 💪🏼
4 > Pattern match on whatever you need!
5 > Delegate to other functions logic handling
6 > Use wildcards `_` as part of the pattern!
7 > Lists in Reason/OCaml are singly linked lists O(1)
8, 9 > re-use patterns! (Haskell 😭)
10 > Use familiar, JavaScript-like syntax 😉
11 > default pattern!
```

---

export default Layout

## The Elm Architecture (TEA) 🍵

---

export default Layout

### How does an actual ReasonReact component look like? 🤔

---

export default Layout

```reason Reason React ⚛️
[@react.component]
let make = (~posts, ~postId, ~dispatch, ~navigate) =>
  switch (posts->List.getBy(({Types.Post.id}) => id == postId)) {
  | None => <Error />
  | Some(post) =>
    <div className="single-photo">
      <Post post dispatch navigate />
      <Comments postId dispatch comments={post.comments} />
    </div>
  };
----
* > React component in Reason!
1 > Annotation to have a minimalistic React component ⚛️
2[2] > `make` is the equivalent of the `render` function in js...
2[5:20] > (~) named arguments!
7[3], 8[3:9] > JSX punning! Nice uh? 😉
3,6 > Everything is an expression, no need for `return`!
3:5 > Notice this three magic lines...
3[6:8] > What is this `List.getBy()` function? What is its return type??
3[5] > Also, WHAT IS THAT WEIRD `->` IS THIS PHP!?!?!? (coming soon...)
```

---

export default Layout

```reason
let getBy: (list('a), 'a => bool) => option('a);

// ...

type option('a) = None | Some('a);
----
1 > type signature of getBy... 🔍
3 > that's called `find` in JS! Why it's not called the same!? 🤷🏼‍♂️
1[19:23] > notice the difference!! It returns `option('a)`...
5 > Mmmhh... this sounds familiar!! 🤔
5[2:6] > Yes, `option()` is *Maybe* in Haskell! 🤯
5[4:5] > 'a is a type argument (polymorphic)
5[10] > `None` represents the absence of value... (Nothing)
5[14] > ...and `Some` the presence of the type... (Just)
5[15:18] > ...that contains the desired value!
```

---

export default Layout

Let's see what happens if we don't use `option()`...

---

export default Layout

### Downsides of Elm...

<Notes>
  <li>Only Frontend</li>
  <li>Not really FFI (Foreign Function Interface)</li>
  <li>Small and niche... (not mainstream)</li>
  <li>Weird syntax for beginners...</li>
</Notes>

---

export default Layout

## What was that strange pipe? 😨

`|>` pipe first vs. `->` fast pipe! 🚀

---

export default Layout

> Make impossible states _actually_ impossible!

---

export default Layout

```reason Real World Example! 🌎
type alias Model =
    { xtoken : XToken
    , today : Maybe Date
    , pageModel : PageModel
    }


type PageModel
    = Editor EditorModel
    | Loading LoadingModel
    | Successful SuccessfulModel
    | Failure FailureModel
    | NoDataYet
----
* > Coding suddenly becomes modelling your data with the proper data structure!!
```

---
export default Split

## Compiler Error Messages! 🤩

<img src={require('file-loader!./img/error1.webp')} />

---
export default Layout

## Compiler Error Message: Type Mismatch 💔

<img width="1000px" src={require('file-loader!./img/error2.png')} />

---
export default Layout

## Compiler Error: Typo Checking! 🐛

<img width="800px" src={require('file-loader!./img/error3.png')} />

---
export default Layout

## Compiler Error: Missing Patterns! 🪩

<img width="1000px" src={require('file-loader!./img/error4.png')} />

---
export default Layout

```reason Use your favourite hooks! 🥰
type action =
  | Tick;

type state = {
  count: int,
};

[@react.component]
let make = () => {
  let (state, dispatch) = React.useReducer(
    (state, action) =>
      switch (action) {
      | Tick => {count: state.count + 1}
      },
    {count: 0}
  );

  React.useEffect0(() => {
    let timerId = Js.Global.setInterval(() => dispatch(Tick), 1000);
    Some(() => Js.Global.clearInterval(timerId))
  });

  <div>{ReasonReact.string(string_of_int(state.count))}</div>;
};
----
* > Does this sound familiar?
10:15 > useReducer() with pattern matching! 🤓
18:21 > do side effects safely with useEffect() ⏳
20 > ️⚠️ Catch: since everything needs to return something, `useEffect()` must return `option()`!
```

---

export default Layout

## Nice Editor Integration!

---

export default Layout

## Honorable Mentions:

---

export default Layout

`elm-community/webgl`

![elm-webgl](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*dSY9TyIITI2jd0sJTgwbUQ.gif)

_"Type once, use forever!" -Cristiano Calcagno_

---

export default Layout

`ianmackenzie/elm-3d-scene`

![elm-3d-scene](https://ianmackenzie.github.io/elm-3d-scene/images/1.0.0/hable-filmic-tone-mapping-bright.png)

A game in Reason that compiles to **native** iOS and **native** Android! 😱

---

export default Layout

# Thanks! 😸

---

<Image src={require('file-loader!./img/ending.png')} />
